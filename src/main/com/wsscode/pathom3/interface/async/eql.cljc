...
(defn process-ast* [env ast]
  (p/let [ent-tree* (get env ::p.ent/entity-tree* (p.ent/create-entity {}))
    ...
    (p.eql/traced-mask-output env result ast)))
...
(>defn process
  ...
  ([env tx]
   ...
   (p/let [env env]
     (process-ast (assoc env ::pcr/root-query tx) (p.eql/traced-query->ast env tx))))
  ([env entity tx]
...
(>defn boundary-interface
  ...
  (let [env' (p/let [env env] (pci/register env pcf/foreign-indexes-resolver))]
    (fn boundary-interface-internal
      ([env-extension input]
       (-> (p/let [env env
                   {:pathom/keys [eql entity ast include-stats?] :as request}
